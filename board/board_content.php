...
/*      조회수 중복방지 쿠키       */
if(isset($num) && !isset($_COOKIE['hit_cookie'.$num])){
    
    $hit_sql = "UPDATE parenting_board SET hit = hit + 1 WHERE num='$num'";
    $result = $mysqli->query($hit_sql);
    
    if(isset($result)){
        setcookie('hit_cookie'.$num, time() + 3600, '/'); 
...
사유에따라 처리시간이 다소 지연될 수 있습니다."></textarea>
                ...
                <p class="report_bottom">
                    ...
                    <input type="hidden" name="val" value="육아">
                </p>
            </form>
        </div>
    </div>
    ...
    <div class="delete_pop">
       <div class="delete_body">
          ...
          <h2>게시글을 삭제하시겠습니까?</h2>
          <div class="de_bottom">
             <a href="delete.php?num=<?=$num?>&val=parenting"><button>삭제하기</button></a>
             <button>취소</button>
          </div>
       </div>
    </div>
...
?>
            ...
            <div class="the_text">
                <a href="board_detail.php?name=<?=$row['name']?>"><span><?=$row['name']?>님의 게시글 더보기<i class="fas fa-angle-right" id="arrow"></i></span></a>
                <?php
                if(isset($_SESSION['user_nickname'])){
                    if($row['name'] == $_SESSION['user_nickname']){
                ?>
                <div class="up_delete">
                    <a href="revise_board.php?num=<?=$num?>&val=parenting"><span id="revise_go">수정</span></a>
...
?>
    ...
    $resql = "SELECT * FROM reply WHERE board_num='$num'";
    ...
    $sql2 = "SELECT * FROM reply WHERE board_num='$num' limit $start,$view_article";
...
?>
            <div class="that_content">
                <div class="con_flex">
                    ...
                    ?>
                    <p class="re_span">
                        <a href="reply_revise.php?id=<?=$re_num?>&num=<?=$num?>&val=parenting"><span class="reply_revise_btn">수정</span></a>
                        <span>|</span>
                        <a href="reply_delete.php?id=<?=$re_num?>&num=<?=$num?>&val=parenting"><span class="reply_delete_btn">삭제</span></a>
                    </p>
                    <?php
                        }
                        else if($_SESSION['user_nickname'] == "관리자"){
...
?>
                ...
                <div class="that_writing">
                    ...
                    <form method="POST" action="reply.php" id="reply">
                        ...
                        <p>
                            ...
                            <input type="hidden" name="num" value="<?=$num?>">
                            <input type="hidden" name="re_num" value="육아">
                        </p>
                    </form>
                </div>
            </div>
    ...
    <div class="board_bottom">
        ...
        <div class="right_btn">
            <a href="parenting_board.php"><button class="list"><span>목록</span></button></a>
            <a href="#">
